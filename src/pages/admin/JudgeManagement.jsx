...
const JudgeManagement = () => {
    ...
    const {
        ...
        flaggedSubmissions, unflagConflict
    } = useJudge();
    ...
    const tabs = [
        ...
        { id: 'conflicts', label: 'Conflicts', icon: AlertCircle, count: (flaggedSubmissions || []).filter(c => c.status === 'Pending').length },
            ...
            <div className="flex gap-2 bg-slate-100 dark:bg-slate-800/50 rounded-2xl p-1.5">
                {tabs.map(t => (
                    ...
                    >
                        ...
                        {t.count > 0 && (
                            <span className="flex items-center justify-center h-4 w-4 rounded-full bg-amber-500 text-[10px] text-white">
                                {t.count}
                            </span>
                        )}
                    </button>
                ...
                <div className="space-y-4">
                    {(flaggedSubmissions || []).length === 0 ? (
                        <div className="text-center py-16 rounded-2xl border border-dashed border-slate-300 dark:border-slate-700">
                            ...
                            <p className="text-sm font-bold text-slate-600 dark:text-slate-300">No conflicts flagged</p>
                            <p className="text-xs text-slate-400 mt-1">Judges are performing evaluations smoothly</p>
                        </div>
                    ) : (
                        (flaggedSubmissions || []).map((conflict, i) => {
                            const judge = judges.find(j => j.id === conflict.judgeId);
                            const submission = submissions.find(s => s.id === conflict.submissionId);
                            const competition = competitions.find(c => c.id === submission?.competitionId);

                            return (
                                <div key={i} className="p-6 rounded-2xl border bg-white dark:bg-slate-900 border-amber-200 dark:border-amber-900/50 shadow-lg shadow-amber-500/5">
                                    <div className="flex flex-col md:flex-row md:items-center justify-between gap-6">
                                        <div className="space-y-4 flex-1">
                                            <div className="flex items-center gap-3">
                                                <div className="px-2.5 py-1 rounded-lg text-[10px] font-bold uppercase tracking-wider bg-amber-100 text-amber-700 dark:bg-amber-900/30 dark:text-amber-400">
                                                    Pending
                                                </div>
                                                <span className="text-[10px] text-slate-400">
                                                    Flagged on {conflict.date ? new Date(conflict.date).toLocaleDateString() : 'Unknown'}
                                                </span>
                                            </div>

                                            <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
                                                <div>
                                                    <p className="text-[10px] font-bold text-slate-400 uppercase tracking-widest mb-1">Judge</p>
                                                    <div className="flex items-center gap-2">
                                                        <span className="text-lg">{judge?.avatar}</span>
                                                        <span className="text-sm font-bold text-slate-700 dark:text-slate-200">{judge?.name}</span>
                                                    </div>
                                                </div>
                                                <div>
                                                    <p className="text-[10px] font-bold text-slate-400 uppercase tracking-widest mb-1">Project</p>
                                                    <p className="text-sm font-bold text-slate-700 dark:text-slate-200">{submission?.title}</p>
                                                    <p className="text-[10px] text-slate-500">{competition?.name}</p>
                                                </div>
                                            </div>

                                            <div className="p-3 rounded-xl bg-amber-50/50 dark:bg-amber-900/10 border border-amber-100 dark:border-amber-900/20">
                                                <p className="text-[10px] font-bold text-amber-800 dark:text-amber-300 uppercase tracking-widest mb-1">Reason for Flag</p>
                                                <p className="text-xs text-amber-700 dark:text-amber-400 italic">"{conflict.reason}"</p>
                                            </div>
                                        </div>

                                        <div className="flex flex-col gap-2">
                                            <button
                                                onClick={() => unflagConflict(conflict.judgeId, conflict.submissionId)}
                                                className="px-6 py-2.5 rounded-xl bg-emerald-500 text-white text-xs font-bold hover:bg-emerald-600 transition-colors shadow-lg shadow-emerald-500/20"
                                            >
                                                Dismiss Conflict
                                            </button>
                                            <button
                                                onClick={() => {
                                                    unassignJudge(conflict.judgeId, submission?.competitionId);
                                                    unflagConflict(conflict.judgeId, conflict.submissionId);
                                                }}
                                                className="px-6 py-2.5 rounded-xl bg-slate-800 dark:bg-slate-700 text-white text-xs font-bold hover:bg-slate-700 dark:hover:bg-slate-600 transition-colors"
                                            >
                                                Reassign &amp; Resolve
                                            </button>
                                        </div>
